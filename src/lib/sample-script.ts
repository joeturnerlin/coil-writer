export const SAMPLE_SCRIPT_FILENAME = 'Sample_Script.fountain'

export const SAMPLE_SCRIPT_CONTENT = `Title: The Meeting
Credit: A Coil Sample Script
Author: Coil Writer
Draft date: 2026

===

INT. OFFICE LOBBY - MORNING

A sleek corporate lobby. MARCUS CHEN, 40s, sharp suit and RED TIE, sharper eyes, checks his watch. He's been waiting.

MARCUS
I've been here forty minutes. That's not a mistake. That's a message.

The elevator DINGS. DANA REEVES, 30s, confident stride, tablet in hand, doesn't apologize.

DANA
Marcus. The board is ready.

MARCUS
I'm sure they are.

INT. BOARDROOM - CONTINUOUS

Ten faces around a polished table. VICTOR HALL, 60s, silver-haired patriarch, sits at the head. Marcus sets his GOLD PEN on the table.

VICTOR
You wanted this meeting, Marcus. You have fifteen minutes.

MARCUS
I need five.

He drops a folder on the table. Pages slide toward Victor.

MARCUS (CONT'D)
Three offshore accounts. Two shell companies. One signature — yours.

VICTOR
I'm not scared of you.

MARCUS
I'm not trying to scare you. I'm trying to save you from yourself.

DANA
(looking at her tablet)
The SEC filing goes live at noon. We can delay it or we can file it as-is.

VICTOR
You're bluffing. You don't have enough.

MARCUS
I have enough to make your lawyers nervous. That's all I need.

EXT. PARKING GARAGE - LATER

Marcus and Dana walk to their cars. Marcus adjusts his BLUE TIE. The tension bleeds out.

DANA
I was terrified in there. My heart was pounding the whole time.

MARCUS
You didn't show it.

DANA
What happens if he doesn't fold?

MARCUS
Then we file, and the world sees what he's built. I feel really angry about all of this.

DANA
I know the feeling. I'm sad about how things turned out with Victor. He used to be different.

INT. MARCUS'S CAR - NIGHT

Marcus sits alone. Engine off. Streetlight through the windshield.

He reaches for the gold pen — but his pockets are empty. He pulls out his phone. Types a message. Deletes it. Types again.

MARCUS (V.O.)
You know, the real lesson here is that power corrupts everyone eventually. That's really what this whole thing is about — the corruption of good intentions.

He sends the message. Sets the phone down. Closes his eyes.

FADE OUT.
`
